...
const AnnuaireSu = ({ data, filters }: any) => {
	...
	const [selectedCategoriesLength, setSelectedCategoriesLength] =
		useState<any>(0);
	const [isSelected, setIsSelected] = useState<boolean>(false);
	const [currentInput, setCurrentInput] = useState<string>("");
	const [moreFiltersClicked, setMoreFiltersClicked] = useState<boolean>(false);
	...
	const categorieClickHandler = (categorie: any) => {
		let element: HTMLElement | null = document.getElementById(categorie);
		...
		if (selectedCategories.includes(categorie)) {
			...
			if (elementChildren && elementChildren2) {
				...
				elementChildren.style.display = "block";
			}
	...
	return (
		<div className="container">
			<div className="annuaire__searchbar">
				...
				{/* Plus de filtres */}
				<div className="annuaire__searchbar-more-wrapper">
					<p
						className="annuaire__searchbar-more"
						id="moreFilters"
						onClick={() => {
							const moreFilters = document.querySelector(
...
export async function getServerSideProps(context: any) {
  ...
  const { locales, defaultLocale, locale } = context;
	...
	return { props: { data, filters, locales, defaultLocale, locale } };
}
